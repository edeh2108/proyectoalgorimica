            ...
            with st.container():
                ...
                if st.session_state.get(f"rechazando_{p['id']}"):
                    ...
                    if st.button("Confirmar sanción y eliminar solicitud", key=f"confirmar_no_{p['id']}"):
                        db.sancionar_dni(p["dni"], motivo or "Solicitud fraudulenta o inconsistente", int(meses))
                        db.eliminar_pendiente(p["id"])
    ...
    with tabs[1]:
        ...
        if guardar:
            if dni_p and nombres_p and apellidos_p:
                db.registrar_paciente(dni_p, nombres_p, apellidos_p, int(edad_p), tipo_p)
        ...
        if not pacientes:
            ...
            if agendar:
                ...
                if db.existe_conflicto_horario(especialidad_c, fecha_c.isoformat(), hora_c):
                    ...
                    db.crear_cita(dni_sel, especialidad_c, fecha_c.isoformat(), hora_c)
            ...
            for c in citas_dni:
                ...
                if reprogramar:
                    if db.existe_conflicto_horario(
                        ...
                        db.reprogramar_cita(c["id"], nueva_fecha.isoformat(), nueva_hora)
